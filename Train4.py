...
# k7 = data(arraydata[int(random.uniform(0, 687))].x, arraydata[int(random.uniform(0, 687))].y, 0)
...
while 1:
    ...
    for i in range(len(arraydata)):
        ...
        if (temp == tempK1):
            ...
            clusterK7.append(data(arraydata[i].x, arraydata[i].y, 7))

            jumlahX7 += arraydata[i].x
            jumlahY7 += arraydata[i].y

            arraydata[i].cluster = 7

        elif (temp == tempK8):

            clusterK8.append(data(arraydata[i].x, arraydata[i].y, 8))

            jumlahX8 += arraydata[i].x
            jumlahY8 += arraydata[i].y
    ...
    k7 = data(jumlahX7 / len(clusterK7), jumlahY7 / len(clusterK7), 7)
    k8 = data(jumlahX8 / len(clusterK8), jumlahY8 / len(clusterK8), 8)
    ...
    if (k1 == Clusterbaru1 and k2 == Clusterbaru2 and k3 == Clusterbaru3 and k4 == Clusterbaru4 and
                k5 == Clusterbaru5 and k6 == Clusterbaru6 and k7 == Clusterbaru7 and k8 == Clusterbaru8):
        break
...
print "k7 -> x = "+str(k7.x)+" , "+"y = "+str(k7.y)
print "k8 -> x = "+str(k8.x)+" , "+"y = "+str(k8.y)
...
for i in range(8):
    if (i == 1):
        ...
        for j in range(len(clusterK7)):
            total += (((k7.x - clusterK7[i].x) ** 2) + ((k7.y - clusterK7[i].y) **2))
    elif (i == 8):
        for j in range(len(clusterK8)):
            total += (((k8.x - clusterK8[i].x) ** 2) + ((k8.y - clusterK8[i].y) **2))
